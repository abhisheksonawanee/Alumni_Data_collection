...
def init_db():
    ...
    """
    schema_path = BASE_DIR / "schdb.sql"
    ...
    if schema_path.exists():
        ...
        try:
            ...
            # If schema execution fails, try creating essential tables manually
            create_essential_tables(conn)
        finally:
            conn.close()
    else:
        # If no schema file, create essential tables manually
        conn = get_conn()
        create_essential_tables(conn)
        conn.close()


def create_essential_tables(conn):
